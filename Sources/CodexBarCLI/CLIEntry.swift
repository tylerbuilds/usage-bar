...
enum CodexBarCLI {
    ...
    private static func runUsage(_ values: ParsedValues) async {
        ...
        // On Linux, default to OAuth for Claude since CLI probe is broken in v2.0.76+
        #if os(macOS)
        ...
        case .oauth: .oauth
        case .cli: .cli
        case .web, .auto: .oauth  // Linux: use OAuth by default
        }
        #endif
        ...
        #if !os(macOS)
        if sourceMode == .web {
            Self.exit(code: .failure, message: "Error: --source web is only supported on macOS.")
        }
        #endif
        ...
        context: ProviderFetchContext) async -> ProviderFetchOutcome
    {
        ...
        // On Linux, .auto uses CLI/OAuth instead of web, so only try web if explicitly requested
        #if os(macOS)
        let shouldTryCodexWeb = provider == .codex && context.sourceMode.usesWeb
        #else
        let shouldTryCodexWeb = provider == .codex && context.sourceMode == .web
        #endif

        if shouldTryCodexWeb {
        ...
        // On Linux, .auto uses OAuth instead of web, so only try web if explicitly requested
        #if os(macOS)
        let shouldTryClaudeWeb = provider == .claude && context.sourceMode.usesWeb
        #else
        let shouldTryClaudeWeb = provider == .claude && context.sourceMode == .web
        #endif

        if shouldTryClaudeWeb {
            do {
